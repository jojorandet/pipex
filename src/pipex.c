...
/*   Updated: 2025/03/03 18:56:10 by jrandet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pipex.h"

static void	command_init(t_pipex *pipex, t_command *cmd, char *str)
{
	cmd->path = find_command_path(pipex, str);
	if (!cmd->path)
		pipex_exit(pipex, "Error: path could not be built!\n");
